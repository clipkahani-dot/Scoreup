...
function formatMath(text) {
    ...
    // 1. Convert Fractions to professional horizontal layout
    ...
    // Special manual overrides for nested or complex ones from screenshot
    newText = newText.split('(sin θ / cos θ) / (1 - cos θ / sin θ)').join(`<span class="frac"><span class="num">${toFrac('sin θ', 'cos θ')}</span><span class="den">1 - ${toFrac('cos θ', 'sin θ')}</span></span>`);
    newText = newText.split('(cos θ / sin θ) / (1 - sin θ / cos θ)').join(`<span class="frac"><span class="num">${toFrac('cos θ', 'sin θ')}</span><span class="den">1 - ${toFrac('sin θ', 'cos θ')}</span></span>`);
    newText = newText.split('(sin θ / cos θ) / ((sin θ - cos θ) / sin θ)').join(`<span class="frac"><span class="num">${toFrac('sin θ', 'cos θ')}</span><span class="den">${toFrac('sin θ - cos θ', 'sin θ')}</span></span>`);
    newText = newText.split('(cos θ / sin θ) / ((cos θ - sin θ) / cos θ)').join(`<span class="frac"><span class="num">${toFrac('cos θ', 'sin θ')}</span><span class="den">${toFrac('cos θ - sin θ', 'cos θ')}</span></span>`);
    newText = newText.split('(sin² θ + cos² θ + sin θ cos θ) / (sin θ cos θ)').join(`<span class="frac"><span class="num">sin² θ + cos² θ + sin θ cos θ</span><span class="den">sin θ cos θ</span></span>`);
    newText = newText.split('(1 / cos θ - sin θ / cos θ)²').join(`(${toFrac('1', 'cos θ')} - ${toFrac('sin θ', 'cos θ')})²`);
    newText = newText.split('((1 - sin θ) / cos θ)²').join(`(${toFrac('1 - sin θ', 'cos θ')})²`);

    newText = newText.replace(/\(√3 \/ 2\)\²/g, `(${toFrac('√3', '2')})²`);
    newText = newText.replace(/√3 \/ 2/g, toFrac('√3', '2'));

    // 2. Put instructions in small brackets and handle newlines with <br>
    // First, identify descriptive lines (usually start with "माना", "दिया है", "चूँकि", "अब", "पदों को", "एलसीएम", "अंश", "सिद्ध", "मान रखने")
    const instructions = [
    ...
    instructions.forEach(ins => {
        newText = newText.split(ins).join(`(${ins})`);
    });

    // 3. Fix literal \n by replacing with <br> and also ensure no double brackets
...
console.log('SUCCESS: Chapter 8 PYQ updated. Removed \n literal and wrapped instructions in brackets.');
