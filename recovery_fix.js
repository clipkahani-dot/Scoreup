...
// Marker for the start of the corruption
// Based on current file state after Turn 21 partial fix
const startSearch = 'Step 5: 2 sec A = R.H.S (इति सिद्धम)"\n        },';
const startIndex = content.indexOf(startSearch);

if (startIndex === -1) {
    console.log("Start marker not found. Trying variation...");
    const altStart = 'Step 5: 2 sec A = R.H.S (इति सिद्धम)"\r\n        },';
    const altIndex = content.indexOf(altStart);
    if (altIndex === -1) {
        console.error("Critical Error: Start marker not found in any form.");
        process.exit(1);
    }
    startIndex = altIndex;
}

const splitIndex = startIndex + (startSearch.includes('\r\n') ? startSearch.length : startSearch.replace('\n', '\r\n').length); 
// Note: Handled carriage return potential

// Marker for the end of the corruption (Start of Chapter 9)
const endSearch = '{\n    "subject": "Mathematics",\n    "class_level": "Class 10",\n    "chapter": "त्रिकोणमिति के कुछ अनुप्रयोग"';
let endIndex = content.indexOf(endSearch);

if (endIndex === -1) {
    console.log("End marker not found with LF. Trying CRLF...");
    const altEnd = endSearch.replace(/\n/g, '\r\n');
    endIndex = content.indexOf(altEnd);
    if (endIndex === -1) {
        console.error("Critical Error: End marker not found.");
        process.exit(1);
    }
}

// Reconstruct the correct Chapter 8 content
const correctC8 = `
      "short": [
        {
...
const finalContent = content.substring(0, splitIndex) + correctC8 + content.substring(endIndex);
fs.writeFileSync(path, finalContent);
console.log("Successfully surgical-repaired math-data.js");
