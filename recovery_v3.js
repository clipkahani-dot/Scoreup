...
// The marker where we fixed line 963
const startMarker = '"Step 5: 2 sec A = R.H.S (इति सिद्धम)"\n        },\n      "short": [';
const startIndex = content.indexOf(startMarker);

if (startIndex === -1) {
    console.log("Start marker not found. Trying variation...");
    // Fallback if formatting differs
    const fallbackMarker = '"Step 5: 2 sec A = R.H.S (इति सिद्धम)"';
    const fallbackIndex = content.indexOf(fallbackMarker);
    if (fallbackIndex === -1) {
        console.error("Critical: Could not find start marker.");
        process.exit(1);
    }
    // Find the end of the current question object
    const nextBrace = content.indexOf('}', fallbackIndex);
    const nextComma = content.indexOf(',', nextBrace);
    // We want to start replacing right after the comma and newline
    startIndex = nextComma + 2; 
} else {
    // If exact match found, we replace starting from '"short": ['
    startIndex += '"Step 5: 2 sec A = R.H.S (इति सिद्धम)"\n        },\n'.length;
}

// The end of Chapter 8 is the start of Chapter 9
const endMarker = '{\n    "subject": "Mathematics",\n    "class_level": "Class 10",\n    "chapter": "त्रिकोणमिति के कुछ अनुप्रयोग"';
const endIndex = content.indexOf(endMarker);

if (endIndex === -1) {
    console.error("Critical: Could not find end marker (Chapter 9 start).");
    process.exit(1);
}

// Reconstruct the correct Chapter 8 content from that point
const replacement = `        {
...
fs.writeFileSync('math-data.js', newContent);
console.log('Math data recovered and restored successfully.');
